 ...
 *          to all entrants whose lottery outcome is SELECTED or ACCEPTED for
...
public class NotifySelectedEntrantsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Update recipient info label to reflect the correct audience
        TextView tvRecipientCount = findViewById(R.id.tv_recipient_count);
        tvRecipientCount.setText("Sending to all selected and accepted entrants");
     ...
     *  1. Their registrationHistory outcome for this event is SELECTED or ACCEPTED, AND
        ...
        for (String userId : waitingListIds) {
            db.collection("users").document(userId).get()
                    .addOnSuccessListener(userDoc -> {
                        boolean isSelectedOrAccepted = false;
                            ...
                            for (Object item : (List<?>) rawHistory) {
                                if (item instanceof Map) {
                                    ...
                                    if (eventId.equals(record.get("eventId"))) {
                                        ...
                                        isSelectedOrAccepted =
                                                "SELECTED".equals(outcome) || "ACCEPTED".equals(outcome);
                                        break;
                                    }
                                }
                            }
        ...
        if (recipientIds.isEmpty()) {
            runOnUiThread(() -> {
                new android.app.AlertDialog.Builder(this)
                        ...
                        .setMessage("There are no selected or accepted entrants to notify, or all have opted out of notifications.")
    ...
    private void showConfirmationAndFinish(int count) {
        new android.app.AlertDialog.Builder(this)
                ...
                .setMessage("Your message was sent to " + count
                        + " selected/accepted entrant" + (count == 1 ? "" : "s") + ".")
