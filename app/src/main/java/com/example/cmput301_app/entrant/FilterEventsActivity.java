 ...
 * Design Pattern: Standard Android Activity with startActivityForResult pattern.
 */
 ...
 *
 * Filters are returned to the calling activity via Intent result.
 */
public class FilterEventsActivity extends AppCompatActivity {
    ...
    // Day chips (mapped to Calendar constants: Sun=1, Mon=2, ..., Sat=7)
    ...
    /** Restore previous filter selections from Intent extras (if any). */
    private void restoreFilterState(Intent intent) {
        ...
        if (days != null) {
            for (int day : days) {
                chipForDay(day).setChecked(true);
            }
        }
    ...
    /** Count how many events match the current filter selections. */
    ...
    /**
     * Applies all active filters to the given list and returns matching events.
     */
    private List<Event> applyFilters(List<Event> events) {
        ...
        for (Event event : events) {
            ...
            if (anyDaySelected) {
                ...
                if (eventDay == -1) continue; // no date
                boolean dayMatch = false;
                for (int d : selectedDays) {
                    if (d == eventDay) { dayMatch = true; break; }
                }
    ...
    /** Collect selected days as Calendar constants. */
    ...
    /** Return a Chip reference for a Calendar day constant. */
    ...
    private void resetAll() {
        ...
        switchHideFull.setChecked(false);

        updateMatchCount();
    }

    /** Packages current selections into Intent extras and finishes with RESULT_OK. */
